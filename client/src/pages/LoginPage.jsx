...
const LoginPage = () => {
  ...
  return (
    ...
    <form onSubmit={submitHandler} action="" className='border-2 bg-white/8 text-white  border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
     ...
     {
      currState === "Sign up" && subm && (
        <textarea onChange={(e)=>setBio(e.target.value) } value={bio}
        rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' placeholder='Enter your Bio...' required></textarea>
      )
     }
     ...
     {currState === "Sign up" ? (
      <p className='text-sm text-gray-500'>
        ...
        className='font-medium text-violet-500 cursor-pointer ml-2'>Login here</span>
      </p>
     ):(<p className='text-sm text-gray-500'>
      Create an account. <span onClick={()=>{ 
        setCurrState("Sign up");
      }} 
      className='font-medium text-violet-500 cursor-pointer ml-2'>Click here</span>
      </p>)
      }
     </div>
