...
// it handles the sending the messge
...
useEffect(()=>{
  ...
  return selectedUser ? (
    ...
    {messages.map((msg,index)=>(
      <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse '}`}>
        {msg.image ? (
          ...
          <p className={`p-2 max-w-[200px] md:text-sm font-light mb-8 break-all bg-violet-500/30 text-white
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden rounded-xl'>
      ...
      <p className='text-lg font-medium text-white'>BaatCheet on TOP</p>
    </div>
  )
}
